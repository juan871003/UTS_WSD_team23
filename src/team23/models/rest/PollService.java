...
public class PollService {
	...
	public ArrayList<Poll> getPollCreator(@QueryParam("username") String username, @QueryParam("status") String status,
		...
		if (username != null && status != null && minResponses != null) {
			...
			if (username != null) {
				...
				for (Poll poll : allPolls) {
					if (poll.getStatus().equals(status)) {
						if ((poll.getPollResponses().size() >= Integer.parseInt(minResponses))
								&& !inList(polls, poll)) {
							polls.add(poll);
						}
					}
